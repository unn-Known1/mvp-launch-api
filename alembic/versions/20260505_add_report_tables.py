...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('id', UUID(as_uuid=True), primary_key=True),
        sa.Column('user_id', UUID(as_uuid=True), sa.ForeignKey('users.id'), nullable=False),
        sa.Column('template_id', UUID(as_uuid=True), sa.ForeignKey('report_templates.id'), nullable=True),
        sa.Column('name', sa.String(255), nullable=False),
        sa.Column('description', sa.Text(), nullable=True),
    ...
    op.create_table(
        'report_deliveries',
        sa.Column('id', UUID(as_uuid=True), primary_key=True),
        sa.Column('scheduled_report_id', UUID(as_uuid=True), sa.ForeignKey('scheduled_reports.id'), nullable=False),
    ...
    op.create_index('ix_report_deliveries_scheduled_report_id', 'report_deliveries', ['scheduled_report_id'])
